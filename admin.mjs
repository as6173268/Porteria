...
/**
 * Script de administración local para Porterias
 ...
 *   node admin.mjs commit --message "Add new strip"
 * 
 * Configuración:
 *   Requiere ADMIN_GH_TOKEN en variables de entorno para commits automáticos
 *   Target repository: albertomaydayjhondoe/Porterias
 */
...
async function addStrip(args) {
  ...
  if (imagePath) {
    newStrip.image_url = `/Porterias/strips/${imagePath}`;
    newStrip.media_type = detectMediaType(imagePath);
  }

  if (videoPath) {
    newStrip.video_url = `/Porterias/strips/${videoPath}`;
    newStrip.media_type = 'video';
    if (!imagePath) {
      // Si no hay imagen, usar el video como imagen también
      newStrip.image_url = `/Porterias/strips/${videoPath}`;
    }
  }

  if (audioPath) {
    newStrip.audio_url = `/Porterias/strips/${audioPath}`;
    newStrip.media_type = 'audio';
    if (!imagePath) {
      newStrip.image_url = `/Porterias/strips/audio-placeholder.png`;
    }
  }
...
async function commitChanges(args) {
  const message = args.message || 'Update strips';
  const token = process.env.ADMIN_GH_TOKEN;
  
  if (!token) {
    log('❌ Error: ADMIN_GH_TOKEN no está configurado', 'red');
    log('   Configura el token en las variables de entorno:', 'yellow');
    log('   export ADMIN_GH_TOKEN="tu_token_aqui"', 'yellow');
    log('\n   O añádelo en GitHub Settings → Secrets → Actions', 'yellow');
    log('   Nombre: ADMIN_GH_TOKEN', 'yellow');
    return;
  }

  try {
    log('\n🔄 Committing changes to albertomaydayjhondoe/Porterias...', 'cyan');
    
    // Aquí se implementaría la lógica de commit usando la API de GitHub
    // Por ahora, solo mostramos instrucciones
    log('⚠️  Función de commit automático en desarrollo', 'yellow');
    log('\n📝 Por ahora, haz commit manualmente:', 'yellow');
    log('   git add public/data/strips.json public/strips/', 'cyan');
...
function showHelp() {
  log('\n📖 Porterias Admin - Gestión Local de Tiras', 'cyan');
  ...
  log('\n  commit   Hacer commit a GitHub (requiere ADMIN_GH_TOKEN)', 'green');
  log('           --message "mensaje"     (opcional)', 'reset');
  ...
  log('  node admin.mjs commit --message "Add new strip"', 'cyan');
  log('');
}
